...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    x_spacing = (w-2*border-1)//(nPointsX-1)
    y_spacing = (h-2*border-1)//(nPointsY-1)
    
    y_coordinates = np.arange(border,h-border,y_spacing)
    x_coordinates = np.arange(border,w-border,x_spacing)
    
    Y, X = np.meshgrid(y_coordinates,x_coordinates)
    
    Y = np.reshape(Y, (nPointsX*nPointsY))
    X = np.reshape(X, (nPointsX*nPointsY))
    
    vPoints = np.c_[Y,X]  # numpy array, [nPointsX*nPointsY, 2]
    return vPoints
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    for i in range(len(vPoints)):
        #TODO
        descriptor_this_gridpoint = np.zeros(0)
        
        for l in range(-2,2):
            for m in range(-2,2):
                y_idx = np.arange(vPoints[i,0]+l*h,vPoints[i,0]+(l+1)*h,1)
                x_idx = np.arange(vPoints[i,1]+m*w,vPoints[i,1]+(m+1)*w,1)
                unwrapped_phase = np.arctan2(grad_y[y_idx,x_idx],grad_x[y_idx,x_idx])%(2*np.pi)
                #Convert unwrapped_phase to integer values in 0,1,2,3,...8(8 is included)
                phase_int = (np.round(unwrapped_phase*4/np.pi)).astype(np.intc)
                
                #0 and 8 correspond to same rounded phase, so replace 8 with 0.
                
                ii = np.where(phase_int==8)
                phase_int[ii] = 0
                hist_ = np.histogram(phase_int,bins=8)[0]
                
                descriptor_this_gridpoint = np.append(descriptor_this_gridpoint,hist_)
        
        descriptors.append(descriptor_this_gridpoint)
...
def bow_recognition_nearest(histogram,vBoWPos,vBoWNeg):
    ...
    """
    Idx_pos, DistPos = findnn(histogram, vBoWPos)
    Idx_neg, DistNeg = findnn(histogram, vBoWNeg)
