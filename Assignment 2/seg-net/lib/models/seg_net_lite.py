...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        in_channel_sizes_down = [input_size]+down_filter_sizes[:-1]
        in_channel_sizes_up = [down_filter_sizes[-1]]+up_filter_sizes[:-1]
        ...
        for i in range(self.num_down_layers):
            layers_conv_down.append(nn.Conv2d(in_channels=in_channel_sizes_down[i],
        ...
        for i in range(self.num_up_layers):
            layers_conv_up.append(nn.Conv2d(in_channels=in_channel_sizes_up[i],
    ...
    def forward(self, x):
        ...
        #Upsampling
        for i in range(self.num_up_layers):
            y = self.layers_unpooling[i](y,maxpool_indices[-(i+1)])
