...
def EstimateEssentialMatrix(K, im1, im2, matches):
  ...
  homo_kps1 = np.append(kps1, np.ones((kps1.shape[0], 1)), 1)
  homo_kps2 = np.append(kps2, np.ones((kps2.shape[0], 1)), 1)
  
  K_inv = np.linalg.inv(K)
  
  normalized_kps1 = (K_inv@homo_kps1.T).T
  normalized_kps1 = normalized_kps1/normalized_kps1[:,-1,None]
  normalized_kps2 = (K_inv@homo_kps2.T).T
  normalized_kps2 = normalized_kps2/normalized_kps2[:,-1,None]
  ...
  for i in range(matches.shape[0]):
    # TODO
    # Add the constraints
    constraint_matrix[i] = np.reshape(np.outer(normalized_kps2[matches[i,1]],normalized_kps1[matches[i,0]]),(9))
...
def TriangulatePoints(K, im1, im2, matches):
  ...
  homogeneous_point_3D = np.append(points3D,np.ones((points3D.shape[0],1)),1)
  
  projected_to_1 = P1@homogeneous_point_3D.T
  projected_to_2 = P2@homogeneous_point_3D.T
  
  idx_in_front_of_1 = np.where(projected_to_1[2]>0)
  idx_in_front_of_2 = np.where(projected_to_2[2]>0)
  
  idx_in_front_of_both = np.intersect1d(idx_in_front_of_1,idx_in_front_of_2)
  
  
  points3D = points3D[idx_in_front_of_both]
  im1_corrs = im1_corrs[idx_in_front_of_both]
  im2_corrs = im2_corrs[idx_in_front_of_both]
...
def EstimateImagePose(points2D, points3D, K):  
  ...
  K_inv = np.linalg.inv(K)
  points2D_homo = np.append(points2D, np.ones((points2D.shape[0], 1)), 1)
  
  normalized_points2D = (K_inv@points2D_homo.T).T
  normalized_points2D = normalized_points2D/normalized_points2D[:,-1,None]
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  for reg_image_name in registered_images:
      ...
      points3D_new, corrs_new_reg_image, corrs_new_image = TriangulatePoints(K, images[reg_image_name], image, e_matches)
      corrs[reg_image_name] = [corrs_new_reg_image,np.arange(points3D.shape[0],points3D.shape[0]+len(points3D_new))]
      points3D = np.append(points3D,points3D_new,0)
      corrs_image = np.append(corrs_image, corrs_new_image,0)
  ...
  corrs[image_name] = [corrs_image,np.arange(len(points3D))]
