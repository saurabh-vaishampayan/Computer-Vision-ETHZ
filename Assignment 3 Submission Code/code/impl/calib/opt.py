...
def ReprojectionError(P, point3D, point2D):
    ...
    p2d_projected = P@np.append(point3D,1)
    p2d_projected = p2d_projected[0:2]/p2d_projected[2]
    # Make sure to properly normalize homogeneous coordinates.
    
    return point2D-p2d_projected
...
def OptimizeProjectionMatrix(P, points2D, points3D):
    ...
    print('Reprojection error before start of nonlinear optimization ',f(P))
    
    def print_err(*args, **kwargs):
        x = args[0]
        print('Reprojection error during current iteration :')
        print(np.linalg.norm(ImageResiduals(np.reshape(x, (3, 4)), points2D, points3D)) ** 2)
    
    # Make sure the scale constraint is fulfilled at the beginning
    result = spo.minimize(f, np.reshape(P / P[2,3], 12), options={'disp': True}, constraints=[scale_constraint], callback=print_err,tol=1e-12)
